...
resource "local_file" "bastion_rsa" {
  ...
  filename        = "${path.root}/tmp/s4hana_distributed/bastion_rsa"
...
resource "local_file" "hosts_rsa" {
  ...
  filename        = "${path.root}/tmp/s4hana_distributed/hosts_rsa"
  file_permission = "0400"
}
...
resource "null_resource" "ansible_exec" {
  ...
  provisioner "local-exec" {
    ...
    if [ $(echo $ansible_2_9_version | awk -F '.' '{ printf "'$search_string'", $1,$2,$3,$4; }') -gt $(echo "2.9.0" | awk -F '.' '{ printf "'$search_string'", $1,$2,$3,$4; }') ] && [ $(echo $ansible_2_9_version | awk -F '.' '{ printf "'$search_string'", $1,$2,$3,$4; }') -lt $(echo "2.11.0" | awk -F '.' '{ printf "'$search_string'", $1,$2,$3,$4; }') ]; then
      ...
      curl -L https://github.com/sap-linuxlab/community.sap_install/archive/refs/tags/1.2.2.tar.gz -o ${path.root}/tmp/s4hana_distributed/sap_install-1.2.2.tar.gz
      mkdir -p ${path.root}/tmp/s4hana_distributed/ansible_collections/community/sap_install
      tar xvf ${path.root}/tmp/s4hana_distributed/sap_install-1.2.2.tar.gz --strip-components=1 -C ${path.root}/tmp/s4hana_distributed/ansible_collections/community/sap_install
      curl -L https://github.com/sap-linuxlab/community.sap_operations/archive/refs/heads/main.tar.gz -o ${path.root}/tmp/s4hana_distributed/sap_operations-main.tar.gz
      mkdir -p ${path.root}/tmp/s4hana_distributed/ansible_collections/community/sap_operations
      tar xvf ${path.root}/tmp/s4hana_distributed/sap_operations-main.tar.gz --strip-components=1 -C ${path.root}/tmp/s4hana_distributed/ansible_collections/community/sap_operations
      ...
      ansible-galaxy collection install --requirements-file ${path.module}/ansible_requirements_collections.yml --collections-path ${path.root}/tmp/s4hana_distributed
      #ansible-galaxy install --role-file ${path.module}/ansible_requirements_roles.yml --roles-path ${path.root}/tmp/s4hana_distributed/roles
      # Must export the Shell Variable, otherwise it cannot be read by Ansible binaries
