...
# Note: Disks may come provisioned into partitioned volumes by IBM Cloud and will need to be erased/cleared first prior to configuration.
...
function lvm_filesystem_runner() {
    ...
    ####
    # IBM Power VS other storage notes and commands
    ####
    #
    # IBM Power VS: IF multipath_component_detection in /etc/lvm/lvm.conf is set to = 1 (enable)
    # this results in LVM pvcreate error:
    #   Cannot use /dev/sdX: device is a multipath component
    #
    # IBM Power VS: IF multipath_component_detection in /etc/lvm/lvm.conf is set to = 0 (disable)
    # this results in LVM pvcreate error:
    #  Can't open /dev/sdX exclusively.  Mounted filesystem?
    #  Can't open /dev/sdX exclusively.  Mounted filesystem?
    #
    # IBM Power VS: Show multipath devices:
    # multipath -l
    #
    # IBM Power VS: Map Disk Volumes to Device Mapper (DM) Multipath
    # multipathd show maps json | jq --raw-output '.maps[] | [.path_groups[].paths[].dev, .sysfs, .uuid] | @sh' | tr -d \'\"
    #
    # Each Block Storage volume has 1 Multipath DM, which contains 2 path groups each with 4 paths (i.e. total of 8 device paths for 1 volume, such as /dev/sdb to /dev/sdi)
    ...
    do
      ...
      then
        ...
        echo "IBM Power VS: Detecting Multipath DM"
        multipath_map=$(multipathd show maps json | jq --raw-output '.maps[] | [.path_groups[].paths[].dev, .sysfs, .uuid] | @sh' | tr -d \'\" | sed "s/^/'/" | sed "s/ dm/',dm/" | sed "s/\(.*\) /\1,/")
        multipath_dm_name=$(printf "$multipath_map" | awk -F',' "/$disk_id/ { print \$2 }")
        echo "/dev/$disk_id is multipath /dev/$multipath_dm_name, using this for pvcreate"
        pvcreate "/dev/$multipath_dm_name" --dataalignment $lvm_pv_data_alignment
        echo "Adding /dev/$multipath_dm_name to a list for the LVM Volume Group for $mount_point"
        lvm_volume_group_target_list=$(echo "/dev/$multipath_dm_name" & echo $lvm_volume_group_target_list)
    ...
    lvm_volume_group_target_list=$(echo $lvm_volume_group_target_list | tr ' ' '\n' | sort -u)
...
function physical_volume_partition_runner() {
    ...
    do
      ...
      then
        ...
        multipath_map=$(multipathd show maps json | jq --raw-output '.maps[] | [.path_groups[].paths[].dev, .sysfs, .uuid] | @sh' | tr -d \'\" | sed "s/^/'/" | sed "s/ dm/',dm/" | sed "s/\(.*\) /\1,/")
        multipath_dm_name=$(printf "$multipath_map" | awk -F',' "/$disk_id/ { print \$2 }")
        echo "/dev/$disk_id is multipath /dev/$multipath_dm_name, using this for parted and mkfs"
        exist_test=$(parted --machine --script /dev/$multipath_dm_name print)
        if [[ ! -z $(echo $exist_test | grep $filesystem_format) ]]; then echo 'Already exists, skipping partition creation and formatting...' && continue ; else echo 'Partition Table does not contain physical partition, executing...' ; fi
        parted --script /dev/$multipath_dm_name \
          mklabel gpt \
        ...
        partition_id=$(lsblk /dev/$disk_id -o NAME,TYPE --raw | awk "/part/ { print \$1 }" | awk '!/sd/')
        echo "Disk Partition ID = $partition_id"
        mkfs.$${filesystem_format} -f -b size=$block_size_definition /dev/mapper/$partition_id
        echo "Write Mount Points to Linux File System Table"
        # PhysicalDiskUUID=$(blkid /dev/$multipath_dm_name -s PTUUID -o value)
        echo "/dev/mapper/$partition_id $mount_point $${filesystem_format} $mount_options 0 0"\ >> /etc/fstab
...
function main() {
  ...
  echo 'Install jq'
  if [ "$os_type" = "rhel" ] ; then yum --assumeyes --debuglevel=1 install jq ; elif [ "$os_type" = "sles" ] ; then zypper install --no-confirm jq ; fi
  #web_proxy_ip_port=$(echo ${var.module_var_web_proxy_url} | awk -F '^http[s]?://' '{print $2}')
  #if [ ! -f /usr/local/bin/jq ]; then curl -L --proxy $web_proxy_ip_port 'https://github.com/stedolan/jq/releases/download/jq-1.6/jq-linux64' -o jq && chmod +x jq && mv jq /usr/local/bin; fi
