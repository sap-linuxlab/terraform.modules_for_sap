...
function lvm_filesystem_runner() {
    ...
    ####
    # MS Azure other storage notes and commands
    ####
    #
    # Output IMDS to JSON
    #curl -H Metadata:true --noproxy "*" "http://169.254.169.254/metadata/instance/compute?api-version=2020-09-01" > imds_output.json
    #
    # Output IMDS to single string
    #curl -H Metadata:true --noproxy "*" "http://169.254.169.254/metadata/instance/compute/vmSize?api-version=2020-09-01&format=text"
    #
    # Check Azure VM LUNs
    #ls -lha /dev/disk/azure/scsi1/lun*
    #
    # AZURE: IF multipath_component_detection in /etc/lvm/lvm.conf is set to = 1 (enable)
    # this results in LVM pvcreate error:
    #   Cannot use /dev/sdX: device is a multipath component
    #
    # AZURE: IF multipath_component_detection in /etc/lvm/lvm.conf is set to = 0 (disable)
    # this results in LVM pvcreate error:
    #  Can't open /dev/sdX exclusively.  Mounted filesystem?
    #  Can't open /dev/sdX exclusively.  Mounted filesystem?
    #
    # AZURE: Show multipath devices:
    # multipath -l
    #
    # AZURE: Map Disk Volumes to Device Mapper (DM) Multipath
    ...
    do
      ...
      then
        ...
        then
          ...
          multipath_dm_name=$(printf "$multipath_map" | awk "/$disk_id/ { print \$2 }")
...
function create_swap_file() {
  ...
  echo "Create swapfile (on /usr when using Azure)"
  ...
  dd if=/dev/zero of=/usr/swapfile bs=$swap_calc_bs count=$swap_calc_count
  chmod 600 /usr/swapfile
  mkswap /usr/swapfile
  swapon /usr/swapfile
  echo '/usr/swapfile swap swap defaults 0 0' >> /etc/fstab
  swapon --show
  free -h
