...
resource "null_resource" "dns_resolv_files" {
  ...
  depends_on = [
    azurerm_private_dns_a_record.dns_resource_record_a
  ]
  ...
  #### no changes to resolv.conf, Microsof Azure shared DNS Resolver IP Address is used for Private DNS Resolver IP Address (azureprivatedns.net / 168.63.129.16) ####
...
echo '---- Sleep 30s to ensure host is ready -----' && sleep 30
...
echo 'Running dig @168.63.129.16 A ${var.module_var_host_name}.${var.module_var_dns_root_domain_name}'
dig @168.63.129.16 A ${var.module_var_host_name}.${var.module_var_dns_root_domain_name}

echo '#### Run nslookup ####'
nslookup ${var.module_var_host_name}.${var.module_var_dns_root_domain_name} | \
if [ $? -ne 0 ]
then
  echo '!!! Exit Terraform execution, failed on nslookup resolution of ${var.module_var_host_name}.${var.module_var_dns_root_domain_name}'
else
  nslookup ${var.module_var_host_name}.${var.module_var_dns_root_domain_name}
fi
EOF
