...
function lvm_filesystem_runner() {
    ...
    ####
    # IBM Power LPAR storage notes and commands
    ####
    #
    # IBM Power LPAR storage: IF multipath_component_detection in /etc/lvm/lvm.conf is set to = 1 (enable)
    # this results in LVM pvcreate error:
    #   Cannot use /dev/sdX: device is a multipath component
    #
    # IBM Power LPAR storage: IF multipath_component_detection in /etc/lvm/lvm.conf is set to = 0 (disable)
    # this results in LVM pvcreate error:
    #  Can't open /dev/sdX exclusively.  Mounted filesystem?
    #  Can't open /dev/sdX exclusively.  Mounted filesystem?
    #
    # IBM Power LPAR storage: Show multipath devices:
    # multipath -l
    #
    # IBM Power LPAR storage: Map Disk Volumes to Device Mapper (DM) Multipath
    ...
    do
      ...
      then
        ...
        echo "IBM Power LPAR: Detecting Multipath DM"
...
function main() {
  ...
  printf "\n----------------\n\n"
  echo '--- Rescan SCSI bus for new SCSI/iSCSI devices ---'
