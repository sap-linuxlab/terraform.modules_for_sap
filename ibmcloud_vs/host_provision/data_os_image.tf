...
data "ibm_is_image" "host_os_image" {
  name = reverse(
    sort(
      compact(
        [
          for key, value in
          data.ibm_is_images.host_os_images_list.images :
          length(
            regexall("${var.module_var_host_os_image}", data.ibm_is_images.host_os_images_list.images[key].name)
          ) > 0 ?
          data.ibm_is_images.host_os_images_list.images[key].name
          : ""
        ]
      )
    )
