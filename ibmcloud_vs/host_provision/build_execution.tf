...
resource "null_resource" "execute_os_scripts" {
  ...
  connection {
    ...
    type                = "ssh"
    user                = "root"
    host                = ibm_is_instance.virtual_server.primary_network_attachment[0].virtual_network_interface[0].primary_ip[0].address
