...
resource "ibm_dns_resource_record" "dns_resource_record_a" {
  ...
  rdata       = ibm_is_instance.virtual_server.primary_network_attachment[0].virtual_network_interface[0].primary_ip[0].address
  ttl         = 1800
}
...
resource "ibm_dns_resource_record" "dns_resource_record_ptr" {
  ...
  zone_id     = local.target_dns_zone_id
  type        = "PTR"
  name        = ibm_is_instance.virtual_server.primary_network_attachment[0].virtual_network_interface[0].primary_ip[0].address
