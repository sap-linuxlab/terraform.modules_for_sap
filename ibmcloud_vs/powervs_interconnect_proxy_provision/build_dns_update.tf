...
dig @161.26.0.7 A ${ibm_is_instance.proxy_virtual_server.name}.${var.module_var_dns_root_domain_name}

echo '#### Run nslookup ####'
nslookup ${ibm_is_instance.proxy_virtual_server.name}.${var.module_var_dns_root_domain_name} | \
if [ $? -ne 0 ]
then
  echo '!!! Exit Terraform execution, failed on nslookup resolution of ${ibm_is_instance.proxy_virtual_server.name}.${var.module_var_dns_root_domain_name}'
else
  nslookup ${ibm_is_instance.proxy_virtual_server.name}.${var.module_var_dns_root_domain_name}
fi
EOF
