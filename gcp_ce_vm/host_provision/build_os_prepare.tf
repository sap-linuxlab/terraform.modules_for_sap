...
ip_escaped=$(printf '%q\n' "${google_compute_instance.host.network_interface[0].network_ip}")
set_hostname="${var.module_var_virtual_machine_hostname}.${var.module_var_dns_root_domain_name}"
set_hostname_only="${var.module_var_virtual_machine_hostname}"
...
then
    ...
    nmcli device modify eth0 ipv4.dns-search $set_domain_only,google.internal
    nmcli connection modify Wired\ connection\ 1 ipv4.dns-search $set_domain_only,google.internal

    # Set Hostname and FQDN
    nmcli device modify eth0 ipv4.dhcp-hostname ""
    nmcli device modify eth0 ipv4.dhcp-fqdn $set_hostname
    nmcli connection modify Wired\ connection\ 1 ipv4.dhcp-hostname ""
    nmcli connection modify Wired\ connection\ 1 ipv4.dhcp-fqdn $set_hostname
