...
function lvm_install() {
    ...
    # Install Multipath
    if [ "$os_type" = "rhel" ] ; then yum --disablerepo=*source* --disablerepo=*debug* --disablerepo=*google* --assumeyes --debuglevel=1 install device-mapper-multipath ; elif [ "$os_type" = "sles" ] ; then zypper install --no-confirm multipath-tools ; fi
    ...
    # Install LVM
    if [ "$os_type" = "rhel" ] ; then yum --disablerepo=*source* --disablerepo=*debug* --disablerepo=*google* --assumeyes --debuglevel=1 install lvm2* ; elif [ "$os_type" = "sles" ] ; then zypper install --no-confirm lvm2* ; fi
...
function physical_volume_partition_runner() {
    ...
    do
      ...
      then
        ...
        parted --script /dev/$disk_id mklabel gpt
...
function main() {
  ...
  then
    # Establish Google Filestore Mount via GCP VPC Subnet Private IP
    google_filestore_mount_fqdn_sapmnt='${var.module_var_nfs_boolean_sapmnt ? var.module_var_nfs_fqdn_sapmnt : "null"}'
    ...
    # Mount Google Filestore via IP using NFSv3 - https://cloud.google.com/filestore/docs/mounting-fileshares#linux:-mount
    echo "Mounting NFS for /sapmnt to Google Filestore Mount Target DNS Name: $google_filestore_mount_fqdn_sapmnt"
    #sudo mount -t nfs3 -o nfsvers=3,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=3,resvport,_netdev,rw,intr $google_filestore_mount_fqdn_sapmnt /sapmnt
    echo "# fstab entries for NFS"  >> /etc/fstab
    echo "$google_filestore_mount_fqdn_sapmnt    /sapmnt    nfs    nfsvers=3,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,resvport,_netdev,rw,intr    0    0" >> /etc/fstab
  fi
